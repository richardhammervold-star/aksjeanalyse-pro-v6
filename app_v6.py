...
PRESETS = {
    ...
    "OMX30 (utvalg)": [
        "VOLV-B.ST","ERIC-B.ST","SAND.ST","ATCO-A.ST","ATCO-B.ST","ESSITY-B.ST","SWED-A.ST",
        "SEB-A.ST","ALFA.ST","TELIA.ST","ABB.ST","HEXA-B.ST","SKF-B.ST","BOL.ST","INVE-B.ST",
        "EVO.ST","KINV-B.ST","NDA-SE.ST","MTG-B.ST","SCA-B.ST"
    ],
    ...
    "USA – Teknologi": ["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","AVGO","AMD","CRM","ORCL","NFLX","NOW","INTU","PANW"]
}
...
eps = st.sidebar.number_input("Støyfilter (eps, %)", value=0.2, min_value=0.0, max_value=2.0, step=0.1,
                              help="Kun brukt i grafikk-filter; ikke i label lenger.")
...
def make_label_hard(df: pd.DataFrame, horizon: int) -> pd.Series:
    ...
    y.name = f"Target_{horizon}"
    return y  # kan inneholde NaN i halen; håndteres ved dropna senere
...
def rec_from_prob(prob: float, buy_thr: float, sell_thr: float) -> str:
    if np.isnan(prob):  # ikke modell => HOLD
        return "HOLD"
    if not np.isnan(buy_thr) and prob > buy_thr:
        return "BUY"
    if not np.isnan(sell_thr) and prob < sell_thr:
        return "SELL"
    return "HOLD"
...
if run:
    ...
    try:
        ...
        r1 = rec_from_prob(p1, b1, s1)
        r3 = rec_from_prob(p3, b3, s3)
        r5 = rec_from_prob(p5, b5, s5)
...
if run:
    ...
    with c1:
        ...
        df1 = df1.sort_values("Prob_1d", ascending=False, na_position="last")
    ...
    with c2:
        ...
        df3 = df3.sort_values("Prob_3d", ascending=False, na_position="last")
    ...
    with c3:
        ...
        df5 = df5.sort_values("Prob_5d", ascending=False, na_position="last")
    ...
    cmp_df = df[["Ticker",
                 ...
                 "Delta_5d_1d","Acc","AUC","Composite"]].sort_values(
                 "Composite", ascending=False, na_position="last")

    st.dataframe(style_df(cmp_df, {
